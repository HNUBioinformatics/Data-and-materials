...
list5 = []
for i in dataset:
    list2.append(i[0])  # Sort the first element
    list3.append(i[1])  # Sort the second element
# print(np.sort(list2))
# print(np.sort(list3))
# The main role is to find the value of the matrix row and column
list4 = np.sort(list2)
list5 = np.sort(list3)
A = len(list4)
B = len(list5)
...
for h in range(10):
    ...
    ###Create connectionless data in the test set
    for i in test:
    ...
    Y_2 = []
    # number_ja = 0
    # for h in list_sort_2:
    #     if h[0] != 0:
    #         number_ja += 1
    for i in list_sort_2[0:360]:
        if i[1]==1:
            n_2+=1
        if i[1]==0:
            m_2+=1
    ...
    Y_PA = []
    # number_pa = 0
    # for h in list_sort_PA:
    #     if h[0] != 0:
    #         number_pa += 1
    for i in list_sort_PA[0:360]:
        if i[1]==1:
            n_PA+=1
        if i[1]==0:
            m_PA+=1
...
####Ten times average
list_toall=[]
for j in range(len(list_all_PR[0])):  #
    o = 0
    c = 0
...
list_toall_2 = []
for j in range(len(list_all_PR_2[0])):  #
    o = 0
    c = 0
...
list_toall_PA = []
for j in range(len(list_all_PR_PA[0])):  #
    o = 0
    c = 0
...
#        print(p)
#        print(R)
for i in list_toall_2:
    Y_3.append(i[1])
    X_3.append(i[0])

#        print(p)
#        print(R)
for i in list_toall_PA:
    Y_PA.append(i[1])
    X_PA.append(i[0])
...
test = pd.DataFrame(data=list_toall)
test.to_csv('E:\\DNA\PR and ROC\\AUCN.csv', encoding='gbk')

test = pd.DataFrame(data=list_toall_2)
test.to_csv('E:\\DNA\PR and ROC\\AUJA.csv', encoding='gbk')

test = pd.DataFrame(data=list_toall_PA)
test.to_csv('E:\\DNA\PR and ROC\\AUPA.csv', encoding='gbk')
